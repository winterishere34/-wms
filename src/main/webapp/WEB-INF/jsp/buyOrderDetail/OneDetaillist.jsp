...
<body>
	...
	<script type="text/javascript">
		...
		function to_page(pn){
			$.ajax({
				url:"${APP_PATH}/buyOrderDetail/bo/${boId}",
				data:"pn="+pn,
				type:"GET",
		...
		function build_bos_table(result){
			...
			$.each(bos,function(index,item){
				...
				var bodIdTd = $("<td></td>").append(item.bodId);
				...
				var bodAmountTd = $("<td></td>").append(item.bodAmount);
				var bodBuyPriceTd = $("<td></td>").append(item.bodBuyPrice);
				var bodTotalPriceTd = $("<td></td>").append(item.bodTotalPrice);
				...
				$("<tr></tr>")
					...
					.append(bodAmountTd)
					.append(bodBuyPriceTd)
					.append(bodTotalPriceTd)
					.append(boIdTd)
